...
import { verifyCoOrganizerInvitationRevocation } from "@understoria/shared/crypto";
import type { CoOrganizerInvitationRevocationStore } from "../db.js";
import { parseCoOrganizerInvitationRevocation } from "../validate.js";
...
/**
 * POST /coorg-invitation-revocations
 *   - Body: signed CoOrganizerInvitationRevocation JSON. The inviter
 *     cancels an outstanding invitation before the invitee responds.
 ...
 *
 * GET /coorg-invitation-revocations
 *   - Query: ?since=<ms>&limit=<n>
 *   - Returns the most recent revocations newer than `since`, paginated
 *     by `revokedAt`, capped at `limit` (default 200, hard ceiling
 *     1000). See `docs/co-organizer-invitations.md` §8.
 */
export async function registerCoOrganizerInvitationRevocationRoutes(
  app: FastifyInstance,
  { store }: Deps,
): Promise<void> {
  app.post("/coorg-invitation-revocations", async (req, reply) => {
    const parsed = parseCoOrganizerInvitationRevocation(req.body);
    if (!parsed.ok) {
      reply.code(400);
    ...
    async (req) => {
      ...
      const safeSinceId =
        req.query.sinceId && req.query.sinceId.length > 0
          ? req.query.sinceId
          : undefined;
      const coorgInvitationRevocations = store.list({
        since: safeSince,
        sinceId: safeSinceId,
        limit: safeLimit,
      });
      return {
        count: coorgInvitationRevocations.length,
