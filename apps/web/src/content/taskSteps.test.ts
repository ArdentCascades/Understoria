...
import { PROJECT_TEMPLATES_FIL } from "@/content/projectTemplates.fil";
...
import { TASK_STEPS_UR } from "@/content/taskSteps.ur";
import { TASK_STEPS_ID } from "@/content/taskSteps.id";
import { TASK_STEPS_SW } from "@/content/taskSteps.sw";
import { TASK_STEPS_FIL } from "@/content/taskSteps.fil";
...
describe("TASK_STEPS coverage", () => {
  ...
  it("has one step list per task, index-aligned, in every locale", () => {
    ...
    for (const tpl of PROJECT_TEMPLATES_UR) {
      expect(TASK_STEPS_UR[tpl.id]?.length, `${tpl.id} (ur)`).toBe(
        tpl.tasks.length,
      );
    }
    for (const tpl of PROJECT_TEMPLATES_ID) {
      expect(TASK_STEPS_ID[tpl.id]?.length, `${tpl.id} (id)`).toBe(
        tpl.tasks.length,
      );
    }
    for (const tpl of PROJECT_TEMPLATES_SW) {
      expect(TASK_STEPS_SW[tpl.id]?.length, `${tpl.id} (sw)`).toBe(
        tpl.tasks.length,
      );
    }
    for (const tpl of PROJECT_TEMPLATES_FIL) {
      expect(TASK_STEPS_FIL[tpl.id]?.length, `${tpl.id} (fil)`).toBe(
        tpl.tasks.length,
      );
    }
    for (const tpl of PROJECT_TEMPLATES_BN) {
      expect(TASK_STEPS_BN[tpl.id]?.length, `${tpl.id} (bn)`).toBe(
        tpl.tasks.length,
      );
    }
    for (const tpl of PROJECT_TEMPLATES_HT) {
      expect(TASK_STEPS_HT[tpl.id]?.length, `${tpl.id} (ht)`).toBe(
        tpl.tasks.length,
      );
    }
    for (const tpl of PROJECT_TEMPLATES_FA) {
      expect(TASK_STEPS_FA[tpl.id]?.length, `${tpl.id} (fa)`).toBe(
        tpl.tasks.length,
      );
    }
  ...
  it("gives every task 3-5 steps per locale, with matching counts", () => {
    for (const [id, en] of Object.entries(TASK_STEPS_EN)) {
      en.forEach((list, i) => {
        ...
        expect(TASK_STEPS_UR[id]?.[i]?.length, `${id}[${i}] ur/en counts`).toBe(
          list.length,
        );
        expect(TASK_STEPS_ID[id]?.[i]?.length, `${id}[${i}] id/en counts`).toBe(
          list.length,
        );
        expect(TASK_STEPS_SW[id]?.[i]?.length, `${id}[${i}] sw/en counts`).toBe(
          list.length,
        );
        expect(TASK_STEPS_FIL[id]?.[i]?.length, `${id}[${i}] fil/en counts`).toBe(
          list.length,
        );
        expect(TASK_STEPS_BN[id]?.[i]?.length, `${id}[${i}] bn/en counts`).toBe(
          list.length,
        );
        expect(TASK_STEPS_HT[id]?.[i]?.length, `${id}[${i}] ht/en counts`).toBe(
          list.length,
        );
  ...
  it("keeps every step a short, checkable to-do (non-empty, translated, no essays)", () => {
    for (const [id, en] of Object.entries(TASK_STEPS_EN)) {
      en.forEach((enList, i) => {
        ...
        (TASK_STEPS_UR[id]?.[i] ?? []).forEach((s, j) => {
          expect(s.trim(), `${id}[${i}].ur[${j}]`).not.toBe("");
          expect(s.length, `${id}[${i}].ur[${j}]`).toBeLessThanOrEqual(120);
          expect(s, `${id}[${i}].ur[${j}] ur===en`).not.toBe(enList[j]);
        });
        (TASK_STEPS_ID[id]?.[i] ?? []).forEach((s, j) => {
          expect(s.trim(), `${id}[${i}].id[${j}]`).not.toBe("");
          expect(s.length, `${id}[${i}].id[${j}]`).toBeLessThanOrEqual(120);
          expect(s, `${id}[${i}].id[${j}] id===en`).not.toBe(enList[j]);
        });
        (TASK_STEPS_SW[id]?.[i] ?? []).forEach((s, j) => {
          expect(s.trim(), `${id}[${i}].sw[${j}]`).not.toBe("");
          expect(s.length, `${id}[${i}].sw[${j}]`).toBeLessThanOrEqual(120);
          expect(s, `${id}[${i}].sw[${j}] sw===en`).not.toBe(enList[j]);
        });
        (TASK_STEPS_FIL[id]?.[i] ?? []).forEach((s, j) => {
          expect(s.trim(), `${id}[${i}].fil[${j}]`).not.toBe("");
          expect(s.length, `${id}[${i}].fil[${j}]`).toBeLessThanOrEqual(120);
          expect(s, `${id}[${i}].fil[${j}] fil===en`).not.toBe(enList[j]);
        });
        (TASK_STEPS_BN[id]?.[i] ?? []).forEach((s, j) => {
          expect(s.trim(), `${id}[${i}].bn[${j}]`).not.toBe("");
          expect(s.length, `${id}[${i}].bn[${j}]`).toBeLessThanOrEqual(120);
          expect(s, `${id}[${i}].bn[${j}] bn===en`).not.toBe(enList[j]);
        });
        (TASK_STEPS_HT[id]?.[i] ?? []).forEach((s, j) => {
          expect(s.trim(), `${id}[${i}].ht[${j}]`).not.toBe("");
          expect(s.length, `${id}[${i}].ht[${j}]`).toBeLessThanOrEqual(120);
          expect(s, `${id}[${i}].ht[${j}] ht===en`).not.toBe(enList[j]);
        });
        (TASK_STEPS_FA[id]?.[i] ?? []).forEach((s, j) => {
          expect(s.trim(), `${id}[${i}].fa[${j}]`).not.toBe("");
...
describe("getTaskSteps", () => {
  it("resolves by template + verbatim task title in either locale", () => {
    ...
    const tplUr = PROJECT_TEMPLATES_UR.find((t) => t.id === tpl.id)!;
    const urSteps = getTaskSteps(tpl.id, tplUr.tasks[0].name, "ur");
    expect(urSteps).toEqual([...TASK_STEPS_UR[tpl.id][0]]);
    const tplId = PROJECT_TEMPLATES_ID.find((t) => t.id === tpl.id)!;
    const idSteps = getTaskSteps(tpl.id, tplId.tasks[0].name, "id");
    expect(idSteps).toEqual([...TASK_STEPS_ID[tpl.id][0]]);
    const tplSw = PROJECT_TEMPLATES_SW.find((t) => t.id === tpl.id)!;
    const swSteps = getTaskSteps(tpl.id, tplSw.tasks[0].name, "sw");
    expect(swSteps).toEqual([...TASK_STEPS_SW[tpl.id][0]]);
    const tplFil = PROJECT_TEMPLATES_FIL.find((t) => t.id === tpl.id)!;
    const filSteps = getTaskSteps(tpl.id, tplFil.tasks[0].name, "fil");
    expect(filSteps).toEqual([...TASK_STEPS_FIL[tpl.id][0]]);
    const tplBn = PROJECT_TEMPLATES_BN.find((t) => t.id === tpl.id)!;
    const bnSteps = getTaskSteps(tpl.id, tplBn.tasks[0].name, "bn");
    expect(bnSteps).toEqual([...TASK_STEPS_BN[tpl.id][0]]);
    const tplHt = PROJECT_TEMPLATES_HT.find((t) => t.id === tpl.id)!;
    const htSteps = getTaskSteps(tpl.id, tplHt.tasks[0].name, "ht");
    expect(htSteps).toEqual([...TASK_STEPS_HT[tpl.id][0]]);
