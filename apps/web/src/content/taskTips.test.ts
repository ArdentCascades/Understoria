...
import { TASK_TIPS_FIL } from "@/content/taskTips.fil";
import { TASK_TIPS_BN } from "@/content/taskTips.bn";
...
describe("TASK_TIPS coverage", () => {
  ...
  it("has one tip per task, index-aligned, in every locale", () => {
    ...
    for (const tpl of PROJECT_TEMPLATES_FIL) {
      expect(TASK_TIPS_FIL[tpl.id]?.length, `${tpl.id} (fil)`).toBe(
        tpl.tasks.length,
      );
    }
    for (const tpl of PROJECT_TEMPLATES_BN) {
      expect(TASK_TIPS_BN[tpl.id]?.length, `${tpl.id} (bn)`).toBe(
        tpl.tasks.length,
      );
    }
    for (const tpl of PROJECT_TEMPLATES_HT) {
      expect(TASK_TIPS_HT[tpl.id]?.length, `${tpl.id} (ht)`).toBe(
        tpl.tasks.length,
      );
    }
    for (const tpl of PROJECT_TEMPLATES_FA) {
      expect(TASK_TIPS_FA[tpl.id]?.length, `${tpl.id} (fa)`).toBe(
        tpl.tasks.length,
      );
    }
    for (const tpl of PROJECT_TEMPLATES_MY) {
      expect(TASK_TIPS_MY[tpl.id]?.length, `${tpl.id} (my)`).toBe(
        tpl.tasks.length,
      );
    }
  });
  ...
  it("every tip is non-empty and actually translated", () => {
    for (const [id, tips] of Object.entries(TASK_TIPS_EN)) {
      tips.forEach((tip, i) => {
        ...
        const fil = TASK_TIPS_FIL[id]?.[i] ?? "";
        const bn = TASK_TIPS_BN[id]?.[i] ?? "";
        ...
        expect(fil.trim(), `${id}[${i}].fil`).not.toBe("");
        expect(fil, `${id}[${i}] fil===en`).not.toBe(tip);
        expect(bn.trim(), `${id}[${i}].bn`).not.toBe("");
        expect(bn, `${id}[${i}] bn===en`).not.toBe(tip);
        expect(ht.trim(), `${id}[${i}].ht`).not.toBe("");
        expect(ht, `${id}[${i}] ht===en`).not.toBe(tip);
        expect(fa.trim(), `${id}[${i}].fa`).not.toBe("");
  ...
  it("keeps tips short enough for a task page (no essays)", () => {
    for (const [id, tips] of Object.entries(TASK_TIPS_EN)) {
      tips.forEach((tip, i) => {
        ...
        expect(
          (TASK_TIPS_FIL[id]?.[i] ?? "").length,
          `${id}[${i}].fil`,
        ).toBeLessThanOrEqual(400);
        expect(
          (TASK_TIPS_BN[id]?.[i] ?? "").length,
          `${id}[${i}].bn`,
        ).toBeLessThanOrEqual(400);
        expect(
          (TASK_TIPS_HT[id]?.[i] ?? "").length,
          `${id}[${i}].ht`,
        ).toBeLessThanOrEqual(400);
        expect(
          (TASK_TIPS_FA[id]?.[i] ?? "").length,
          `${id}[${i}].fa`,
        ).toBeLessThanOrEqual(400);
        expect(
...
describe("getTaskTips", () => {
  ...
  it("serves the viewer's language independent of creation language", () => {
    ...
    expect(getTaskTips(tpl.id, tpl.tasks[0].name, "fil")).toBe(
      TASK_TIPS_FIL[tpl.id][0],
    );
    expect(getTaskTips(tpl.id, tpl.tasks[0].name, "bn")).toBe(
      TASK_TIPS_BN[tpl.id][0],
    );
    expect(getTaskTips(tpl.id, tpl.tasks[0].name, "ht")).toBe(
      TASK_TIPS_HT[tpl.id][0],
    );
    expect(getTaskTips(tpl.id, tpl.tasks[0].name, "fa")).toBe(
      TASK_TIPS_FA[tpl.id][0],
    );
