  ...
  return (
    <div className="px-4 pb-fab-clear pt-4">
      ...
      {/* Hidden while the on-screen keyboard is up — the fixed anchor
          ...
          landscape-short pins the pill to the bottom READING-END
          corner for the same reason as lg: centered, it floated over
          the middle of an already short card list; the far edge is the
          one region the reading column and the nav rail never occupy.
          The padding tracks the landscape safe-area inset on THAT
          edge, which is a physical device notch — so the pair goes
          through ltr:/rtl: variants rather than a logical property
          (docs/rtl-plan.md R2). */}
      {!keyboardOpen && !postPanelOpen && (
      <div className="pointer-events-none fixed inset-x-0 bottom-[calc(5rem+env(safe-area-inset-bottom))] z-20 flex justify-center px-4 print:hidden lg:bottom-6 lg:justify-end lg:px-8 landscape-short:bottom-[calc(1rem+env(safe-area-inset-bottom))] landscape-short:justify-end landscape-short:ltr:pr-[max(1rem,env(safe-area-inset-right))] landscape-short:rtl:pl-[max(1rem,env(safe-area-inset-left))]">
