...
| partnership-first | Round one (one per family of North America): Cherokee `chr` (Iroquoian), Navajo `nv` (Na-Dené), Lakota `lkt` (Siouan), Cree `cr` (Algonquian — a continuum; per-dialect entries as communities engage), Inuktitut `iu` (Inuit-Yupik-Unangan). Round two (going global): Choctaw `cho` (Muskogean), Quechua `qu` (Quechuan — a family; per-variety entries as communities engage, es-first fallback), Māori `mi` (Polynesian — also invited to critique the track's own model). Round three: Mohawk `moh` (Iroquoian II), Anishinaabemowin `oj` (Algonquian continuum), Hawaiian `haw` (Polynesian II), plus the FirstVoices/FPCC umbrella (BC's ~34 languages — one conversation, no unbidden scaffolds). Round four: Chickasaw `cic` (Muskogean II), K'iche' `quc` (Mayan), Guaraní `gn` (Tupian), Northern Sámi `se` (Uralic), plus the First Languages Australia and Taiwan Council of Indigenous Peoples umbrellas. Round five: Santali `sat` (Munda — Ol Chiki), Fulfulde `ff-Adlm` (Atlantic — the Adlam script, the track's first African language and fourth RTL), Hmong `mww`/`hnj` (Hmong-Mien — the Mutual Assistance Association diaspora), Western Armenian `hyw` (the first diaspora-endangered entry), plus the INALI/Mexico umbrella (68 language groups). Standing policies: oral-by-choice, access-by-choice, AND threat-model-first — no language scaffolded, probed, or listed unbidden, and for communities where the language itself is treated as suspect by a surveilling state, the first conversation is a threat-model conversation (membership of that category deliberately unenumerated; see the Cherokee partnership doc) | the AI-fleet model is honest for high-resource languages and would not be here — speakers translate, we supply all tooling, and nothing ships before their review. See the `docs/i18n-partnership-*.md` documents and the matching scaffolds in `docs/i18n-glossary/` |
