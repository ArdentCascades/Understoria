...
| push notification (opt-in, docs/notifications.md) | آگاه‌سازی | Deliberately DISTINCT from اعلان, which this app already spent on the board post — the fa/my collision the notifications doc records. آگاه‌سازی (with ZWNJ, rule 8) is the phone-notification register without the state-communiqué smell. DON'T: اعلان (taken — the post), نوتیفیکیشن (loanword), هشدار (an alarm — reserved for real warnings), اطلاعیه (authority notice). |
