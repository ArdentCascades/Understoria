...
| Member-gated reads & writes | Shipped, **default on** (`READ_AUTH`) | Reads require a member signature; attributable writes require a member author. A fresh node boots unclaimed and is claimed with the boot-log setup code. `READ_AUTH=off` is the explicit opt-out for dev/demo nodes |
