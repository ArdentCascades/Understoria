...
| Invite revocation propagation | Modified-consensus ruling on whether revocation withdraws the implicit vouch (design §9) | Revocation is local-only today, so a revoked-then-redeemed invite diverges the newcomer's trust state per device. A signed, authority-bound `InviteRevocation` converges it to one honest `redeemed_despite_revocation` state — never ejection. Convergence half ships ruling-free; trust-withdrawal half needs ratification. Design: [`invite-revocation.md`](./invite-revocation.md) |
