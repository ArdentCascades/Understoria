...
| 8 | (reserved) | — | — |
| 9 | Documentation & i18n | partial | Member/operator/organizer/developer/quickstart guides, political-education. **Eleven languages shipped in full (UI + the whole authored corpus): en, es, fr, pt, zh, hi, vi, ru, ar, bo, ur** — the RTL program (docs/rtl-plan.md R1–R5) carried Arabic and Urdu with a fully mirrored interface, and Tibetan was expedited out of queue order for communities responding to the 2026 Tibet flood; per-language glossaries under docs/i18n-glossary/, program record in docs/i18n-expansion.md, repeatable process in docs/i18n-playbook.md. Nine languages carry reviewStatus: "new" (not yet read by a native speaker — disclosed in Settings) |
