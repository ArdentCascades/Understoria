...
| 8 | (reserved) | — | — |
| 9 | Documentation & i18n | partial | Member/operator/organizer/developer/quickstart guides, political-education. **Fifteen languages shipped in full (UI + the whole authored corpus): en, es, fr, pt, zh, hi, vi, ru, ar, bo, ur, id, sw, fil, bn** — the RTL program (docs/rtl-plan.md R1–R5) carried Arabic and Urdu with a fully mirrored interface, Tibetan was expedited out of queue order for communities responding to the 2026 Tibet flood, and the demand-driven wave added Indonesian, Swahili, Filipino, and Bengali (Bengali also brought the Intl numbering-system pin — intlLocale() in languages.ts); per-language glossaries under docs/i18n-glossary/, program record in docs/i18n-expansion.md, repeatable process in docs/i18n-playbook.md. Thirteen languages carry reviewStatus: "new" (not yet read by a native speaker — disclosed in Settings); native-speaker review is the standing ask |
