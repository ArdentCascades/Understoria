...
| Shift signups (work-party slots) | Operator/community ratification of [`shift-signups.md`](./shift-signups.md) (§14 rulings) | Time-boxed, capped slots on a community event ("Setup crew 9–12, 4 spots") with local-only signups that compose with work days and need gatherings. Zero new wire bytes — shifts and signups are local Dexie rows in the `EventRSVP` posture, so no threat-model widening. The design records one permanent boundary: no event-derived identifier may ever appear in an `Exchange.postId` label (it would rebuild the rejected federated-attendance graph through a side door). Four §14 open questions carry recommended defaults |
