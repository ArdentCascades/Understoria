...
| Composite `(timestamp, id)` federation cursors — phase 3 (phases 1+2 SHIPPED) | The re-seed walker and mirror re-pulls are the batch paths that made this worth finishing; phase 3 is specced and ready | Phases 1+2 shipped: every server store `list()` accepts the optional `sinceId` pair component via the shared `pagedRows` helper, every federation GET route parses it, `db.cursors.test.ts` locks both the wedge fix and the byte-for-byte legacy `since`-only behavior, and the node↔node pullers (`peerPull.ts` — nine kinds, `peer_pull_state` id columns in schema v24; `mirrorPull.ts` used the pair from birth) persist and send the exclusive pair, upgrading legacy NULL-id positions in place. Remaining: phase 3 (PWA pullers persisting `"<ms>:<id>"` with the legacy parse rule) — until it lands, the PWA's pull loops still track bare timestamps and the wedge remains for them, still unreachable through normal one-at-a-time writes. Design: [`composite-federation-cursors.md`](./composite-federation-cursors.md) |
