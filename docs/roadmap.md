...
| Composite `(timestamp, id)` federation cursors | Batch-import/seed tooling that can write more than a page (200 rows) sharing one timestamp — no such tooling exists today | The inclusive `>=` cursors + id/token tiebreaks are tie-safe at page boundaries, but pullers track only `max(timestamp)`: if >limit rows ever share one millisecond, the page fills with the same lowest-id ties every pull and the cursor wedges. Unreachable through normal one-at-a-time writes. The fix — a composite cursor across every store `list()`, every client pull, and `peer_pull_state` — is a wire-protocol change that needs its own design pass, not a rider on a bug-fix PR. Filed at the round-3 review; see the `KNOWN LIMIT` note in `apps/server/src/db.ts` (exchanges `list()`). Design: [`composite-federation-cursors.md`](./composite-federation-cursors.md) (back-compat `sinceId` param, three independently shippable phases) |
