...
| Per-key / per-table insert caps (disk-fill backstop) | A soft-cap policy + operator knob; low urgency behind the rate limiter | A community node accepts any validly-signed federated record (posts, vouches, exchanges, …), and an attacker owns the signing keys they generate — so signature validity is not a scarcity control. Row growth is bounded today only by the global per-minute rate limiter (now per-client behind a proxy, Round-4); there is no per-key or per-table ceiling, so a patient attacker could slowly disk-fill a Raspberry-Pi-class node. Filed at the round-4 review; a real fix (soft caps + pruning policy) needs its own design so it doesn't reject legitimate high-volume communities |
| Signed awaiting-transition artifact (enforce the auto-confirm window server-side) | A federated, signed record of when a post/task entered `awaiting_confirmation` | `POST /auto-confirm` now binds a confirmation to the poster-signed post (`bindToPost` — closes the arbitrary-victim minting hole), but the *age* gate stays advisory: `awaitingSince` is client-supplied and the node holds no signed proof of the transition moment, so a caller can always claim an old value to skip the window. And project-task auto-confirms can't be bound at all (projects don't federate). A small signed "entered awaiting at T" record, verifiable at the endpoint, would make the window enforceable and cover the project-task path. Filed at the round-4 review; see [`auto-confirm-key.md`](./auto-confirm-key.md) §5 |
| Composite `(timestamp, id)` federation cursors — phases 2+3 (phase 1 SHIPPED) | Batch-import/seed tooling remains the urgency trigger; phases 2+3 are specced and ready | Phase 1 shipped: every server store `list()` accepts the optional `sinceId` pair component via the shared `pagedRows` helper, every federation GET route parses it, and `db.cursors.test.ts` locks both the wedge fix (250-row single-millisecond tie converges under the pair cursor) and the byte-for-byte legacy `since`-only behavior across all 12 stores. Remaining: phase 2 (`peer_pull_state` id columns + node↔node pullers sending the pair) and phase 3 (PWA pullers persisting `"<ms>:<id>"` with the legacy parse rule). Until they land, pullers still track bare timestamps — the wedge remains for them, still unreachable through normal one-at-a-time writes. Design: [`composite-federation-cursors.md`](./composite-federation-cursors.md) |
...
| Shift signups — deferred follow-ups (Phase 1 SHIPPED) | Pilot signal; the direct-exchange label needs its own design note | Phase 1 shipped per [`shift-signups.md`](./shift-signups.md) §13 (all four §14 rulings adopted their recommended defaults). What remains deferred: a generic `"direct:<uuid>"` exchange label (credit for plain-event shifts — its own `verifyExchangeLabel` ingestion-scope review), organizer-assisted signup, skill-matched shift discovery ("ways to plug in" note), and co-organizer shift authority on work days. The permanent boundary stands: no event-derived identifier may ever appear in an `Exchange.postId` label (threat-model §7 events entry records it) |
...
- **Dexie migration ordering between parallel branches.** Two
  ...
  *Check:* the next free Dexie version is *reserved* by the first
  ...
  on Project (template-routing), v20 = paired-device inventory
  (`pairingLog`), v21 = co-organizer invitations
  (`coorgInvitations` / `coorgInvitationResponses` /
  `coorgInvitationRevocations`). Server is at v8 (task_comments).
  Next free PWA version: **22**. Next free server version: **9**
  (the co-organizer federation server schema bump lands in PR #176).
